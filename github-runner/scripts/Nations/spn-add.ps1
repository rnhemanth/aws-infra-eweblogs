...
Set-ADServiceAccount -Credential $Credentials -Server $DomainDnsRoot -Identity "$sqlserviceaccount" -ServicePrincipalNames @{Add="MSSQLSvc/$($prefix)RS-01.$($fullDomain):1433", "MSSQLSvc/$($prefix)RS-01.$($fullDomain)", "MSSQLSvc/$($prefix)RS-01.$($fullDomain):$($prefix)RS"}
