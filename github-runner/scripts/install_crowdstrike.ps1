...
Invoke-Command -ComputerName $computerName -Script {
    ...
    $isRunning
    Get-Service | where {($_.Name -like $serviceName)}
    ...
    if ($isRunning.Status -eq "Running") {
        ...
        $Env:PDNumberCaps = $($PDNumber).toUpper() 
        $GroupTag = $($GroupingTag).toUpper() 
        $GroupingTags="AWS_Hosted,$($Environment),$($GroupTag)-$($Env:PDNumberCaps),$($ServerType)"
...
} } -ArgumentList $InstallSource, $CID, $Environment, $ServerType, $PDNumber, $GroupingTag
