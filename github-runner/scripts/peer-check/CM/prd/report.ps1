...
$hash = @{
    ...
    Value = 
		Get-WebBinding -Protocol https | % {
			$_.ItemXPath
			$Thumb = $_.certificateHash
			$iiscert = gci Cert:\LocalMachine\$($_.certificateStoreName) | ? { $_.Thumbprint -eq $Thumb }
			$iiscert.DnsNameList.Unicode
		}
    }
$Output = New-Object psobject -Property $hash
$OSInfo += $Output
...
if ($AutoPageFile -eq $true) {
	$hash = @{
		...
		Value = "Automatically manage paging file size for all drives"
		}
	$Output = New-Object psobject -Property $hash
	$OSInfo += $Output
}
...
#13/05/22 - Changed below due to Emis.Dapi service appearing on DB servers
	...
	(($XMLFilePorts.portcheck.domain | ? {(get-wmiobject WIN32_ComputerSystem).name -match $_.id}).location | ? {$_.name -eq "IndexEndpoints"}).Site | ? {$_.name -eq "S3 DOCS"} |% {$_.port} | foreach {
		$PortTests += New-Object -TypeName PSObject  -Property @{
			Label = "S3 DOCS"
			IP = $_."#text"
			Port = $_.name
