...
Invoke-Command -ComputerName $computerName -Script {
    ...
    if ($isRunning.Status -eq "Running") {
        ...
        Start-Process -FilePath "C:\Program Files\Tenable\Nessus Agent\nessuscli.exe" -ArgumentList "agent link --cloud --key=$Key --proxy-host=$ProxyHostname --proxy-port=$ProxyPort --groups=aws-en-emisweb-$($pdNumberLower)" -Wait
...
} -ArgumentList $InstallSource, $Key, $ProxyHostname, $ProxyPort, $PDNumber
