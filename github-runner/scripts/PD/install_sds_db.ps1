...
Invoke-Command -ComputerName $computerName -Script {
    ...
    if ($isRunning.Status -eq "Running") {
        ...
        C:\Agents\EmisHealthInstaller.exe 'productset=CRDB Slave' action=install machinetype=client patchhubport=33956 $DBProductSet serviceusername=$ServiceUsername servicepassword=$ServicePassword clusterreference=$ClusterReference patchhubaddress=$PatchHubAddress encryptionkey=$Key
