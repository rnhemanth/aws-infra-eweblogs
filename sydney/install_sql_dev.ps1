...
Install-DbaInstance -Version 2019 -SqlInstance "$server\$instance" -Feature Engine,FullText -AdminAccount $AdminAccounts -Configuration $config -Path "C:\Agents\SQL_2019_DEV\" -Confirm:$false -Restart
...
Update-DbaInstance -ComputerName $server -KB $sqlcuversion -Restart -Path "C:\Agents\SQL_2019_Cumulative_Update_27\" -Confirm:$false
Get-DBaService -ComputerName $server | Start-DbaService -Confirm:$false
