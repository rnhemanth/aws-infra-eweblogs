...
data "aws_vpc" "vpc" {
  ...
  tags = {
    Name = "${var.name.environment}-${var.name.service}-net-vpc-${var.name.identifier}"
  }
}
...
data "aws_secretsmanager_secret" "defaultadsecret" {
  arn = var.domain_password_secret_arn
}
...
data "aws_kms_key" "ad_secret" {
  key_id = "alias/${var.name.environment}-${var.kms_service}-sec-kk-${var.name.identifier}"
}