...
module "ec2" {
  ...
  for device in [
    ...
    # T: Drive (if configured)
    try(each.value.t_volume_size, null) != null ? {
      device_name = "/dev/sdt"
      volume_type = "gp3"
      volume_size = each.value.t_volume_size
      throughput  = coalesce(each.value.t_volume_throughput, 125)
      iops        = coalesce(each.value.t_volume_iops, 3000)
      encrypted   = true
      tags = { DriveName = "TEMP_T", DriveLetter = "T" }
    } : null,

    # Q: Drive (if configured)
    try(each.value.q_volume_size, null) != null ? {
      device_name = "/dev/sdq"
      volume_type = "gp3"
      volume_size = each.value.q_volume_size
      throughput  = coalesce(each.value.q_volume_throughput, 125)
      iops        = coalesce(each.value.q_volume_iops, 3000)
      encrypted   = true
      tags = { DriveName = "SQL_INSTANCE", DriveLetter = "Q" }
    } : null,
