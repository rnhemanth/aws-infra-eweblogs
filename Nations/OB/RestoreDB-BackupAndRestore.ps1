...
if (!$(Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISExternalMessaging")) {
    Restore-DbaDatabase -SqlInstance $secondarySqlServer -Path "$($backup_path)\$($s3Path)\EMISExternalMessaging.bak" -Database "EMISExternalMessaging" -WithReplace -DestinationDataDirectory "F:\Databases" -DestinationLogDirectory "F:\Logs"
    Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISExternalMessaging" | Set-DbaDbRecoveryModel -RecoveryModel Full  -Confirm:$false
    Backup-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISExternalMessaging" -FilePath NUL -Type Full
}

if (!$(Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISMonitoring")) {
    Restore-DbaDatabase -SqlInstance $secondarySqlServer -Path "$($backup_path)\$($s3Path)\EMISMonitoring.bak" -Database "EMISMonitoring" -WithReplace -DestinationDataDirectory "E:\Databases" -DestinationLogDirectory "E:\Logs"
    Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISMonitoring" | Set-DbaDbRecoveryModel -RecoveryModel Full  -Confirm:$false
    Backup-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISMonitoring" -FilePath NUL -Type Full
}

if (!$(Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISScheduler")) {
    Restore-DbaDatabase -SqlInstance $secondarySqlServer -Path "$($backup_path)\$($s3Path)\EMISScheduler.bak" -Database "EMISScheduler" -WithReplace -DestinationDataDirectory "E:\Databases" -DestinationLogDirectory "E:\Logs"
    Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISScheduler" | Set-DbaDbRecoveryModel -RecoveryModel Full  -Confirm:$false
    Backup-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISScheduler" -FilePath NUL -Type Full
}

if (!$(Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISEmail")) {
    Restore-DbaDatabase -SqlInstance $secondarySqlServer -Path "$($backup_path)\$($s3Path)\EMISEmail.bak" -Database "EMISEmail" -WithReplace -DestinationDataDirectory "D:\Databases" -DestinationLogDirectory "L:\Logs"
    Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISEmail" | Set-DbaDbRecoveryModel -RecoveryModel Full  -Confirm:$false
    Backup-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISEmail" -FilePath NUL -Type Full
}

if (!$(Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISWebCRTest_$($env_id)_$($pdNumber)")) {
    Restore-DbaDatabase -SqlInstance $secondarySqlServer -Path "$($backup_path)\$($s3Path)\EMISWebCRTest.bak" -Database "EMISWebCRTest_$($env_id)_$($pdNumber)" -WithReplace -DestinationDataDirectory "D:\Databases" -DestinationLogDirectory "L:\Logs"
    Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISWebCRTest_$($env_id)_$($pdNumber)" | Set-DbaDbRecoveryModel -RecoveryModel Full  -Confirm:$false
    Backup-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISWebCRTest_$($env_id)_$($pdNumber)" -FilePath NUL -Type Full
}

if (!$(Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISState")) {
    Restore-DbaDatabase -SqlInstance $secondarySqlServer -Path "$($backup_path)\$($s3Path)\EMISState.bak" -Database "EMISState" -WithReplace -DestinationDataDirectory "D:\Databases" -DestinationLogDirectory "L:\Logs"
    Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISState" | Set-DbaDbRecoveryModel -RecoveryModel Full  -Confirm:$false
    Backup-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISState" -FilePath NUL -Type Full
}

if (!$(Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISWebDocTest_$($env_id)_$($pdNumber)")) {
    Restore-DbaDatabase -SqlInstance $secondarySqlServer -Path "$($backup_path)\$($s3Path)\EMISWebDocTest.bak" -Database "EMISWebDocTest_$($env_id)_$($pdNumber)" -WithReplace -DestinationDataDirectory "M:\Databases" -DestinationLogDirectory "M:\Logs"
    Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISWebDocTest_$($env_id)_$($pdNumber)" | Set-DbaDbRecoveryModel -RecoveryModel Full  -Confirm:$false
    Backup-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISWebDocTest_$($env_id)_$($pdNumber)" -FilePath NUL -Type Full
}
