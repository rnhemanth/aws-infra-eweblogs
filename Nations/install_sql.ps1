...
Install-DbaInstance -Version 2019 -SqlInstance "$server.$ADDomain\$instance" -Feature Engine,FullText -AdminAccount $AdminAccounts -Configuration $config -Path "C:\Agents\SQL_2019_ENT\" -Confirm:$false -Restart
...
Update-DbaInstance -ComputerName "$server.$ADDomain" -KB $sqlcuversion -Restart -Path "C:\Agents\SQL_2019_Cumulative_Update_27\" -Confirm:$false
Get-DBaService -ComputerName "$server.$ADDomain" | Start-DbaService -Confirm:$false
...
$ServiceNameSQLTELEMETRY = 'SQLTELEMETRY$'+$instance
Invoke-Command -ComputerName "$server.$ADDomain" -ScriptBlock {Get-Service $Using:ServiceNameSQLTELEMETRY | Stop-Service -PassThru | Set-Service -StartupType Disabled}