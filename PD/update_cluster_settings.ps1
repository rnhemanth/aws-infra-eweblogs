...
#Remove vote from replica
Write-Host "Setting replica vote settings"
(Get-ClusterNode $RS1).NodeWeight = 0
...
Get-ClusterResource "Cluster Name" | Set-ClusterOwnerNode -Owners $DBS1,$DBS2,$RS1
...
Get-ClusterResource | Where-Object { $_.Name.Contains("Cluster IP Address") } | Where-Object {$_ | Get-ClusterParameter -Name "Address" | Where-Object {$_.Value -eq "$rs_cluster_ip"}} | Set-ClusterOwnerNode -Owners $RS1

#Set AG Resource possible owners
Write-Host "Setting AG resource owner settings"
Get-ClusterResource "$AG1" | Set-ClusterOwnerNode -Owners $DBS1,$DBS2,$RS1
...
Get-ClusterResource ("$AG1" + "_" + "$AG1") | Set-ClusterOwnerNode -Owners $DBS1,$DBS2,$RS1
...
Get-ClusterResource ("$AG1" + "_" + "$AG1subnet2aIP") | Set-ClusterOwnerNode -Owners $RS1
