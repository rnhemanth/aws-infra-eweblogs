...
# restore EMIS Web Doc on DBS01 and DBS02
if (!$(Get-DbaDatabase -SqlInstance $primarySqlServer -Database "EMISWebDocTest_$($pdNumber)")) {
    Restore-DbaDatabase -SqlInstance $primarySqlServer -Path "$($backup_path)\$($s3Path)\EMISWebDocTest.bak" -Database "EMISWebDocTest_$($pdNumber)" -WithReplace -DestinationDataDirectory "D:\Databases" -DestinationLogDirectory "L:\Logs"
}

if (!$(Get-DbaDatabase -SqlInstance $secondarySqlServer -Database "EMISWebDocTest_$($pdNumber)")) {
  Restore-DbaDatabase -SqlInstance $secondarySqlServer -Path "$($backup_path)\$($s3Path)\EMISWebDocTest.bak" -Database "EMISWebDocTest_$($pdNumber)" -WithReplace -DestinationDataDirectory "D:\Databases" -DestinationLogDirectory "L:\Logs"
}

# restore MKBRuntime on DBS01, DBS02 and RS-01
...
Add-DbaAgDatabase -SqlInstance $primarySqlServer -AvailabilityGroup "$($AG2name)" -Database "EMISExternalMessaging", "EMISMonitoring",  "EMISScheduler", "EMISEmail" -SeedingMode Automatic -EnableException
