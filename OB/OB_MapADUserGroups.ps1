...
if ($null -ne $AppServiceName) {
  ...
  $userAppArray = "$AppServiceName-$OU_Name"
  $groupNameArray = "$OU_Name-auth-db","$OU_Name-auth-app","service-accounts"
...
$userSqlArray = "SQLService-$OU_Name","SQLAgent-$OU_Name","SQLBrowser-$OU_Name"
$groupsqlNameArray = "$OU_Name-auth-db"
